...
eventBus.once('headless_wallet_ready', () => {
	...
	eventBus.on('text', async (from_address, text) => {
		...
		if (validationUtils.isValidAddress(ucText)) {
			...
			device.sendMessageToDevice(from_address, 'text', 'Welcome to DICE!\nThe first dice game on the Byteball platform with an available multiplier of 10x and quick betting system. Your task is to choose a number from 10 to 90 and the direction of the forecast (more> / less<). You win if the number falls into the predicted range!\n\nAnd you have a chance to win the Jackpot! Roll 66 twice to get it. The jackpot amount is made up of 0.5% of all losing bets since the last jackpot won.\n\nEmail: xxjenekxx@gmail.com\n[Back](command:menu)');
